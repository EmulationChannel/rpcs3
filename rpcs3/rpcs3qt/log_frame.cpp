...
void log_frame::CreateAndConnectActions()
{
	...
	connect(m_tty_input, &QLineEdit::returnPressed, [this]()
	{
		...
		if (tty && tty->fd)
		{
			std::shared_lock lock(tty->mtx); 
			tty->fd.write(text.c_str(), text.size());
		}
...
void log_frame::UpdateUI()
{
	...
	// Check TTY logs
	if (tty && tty->fd && tty->fd.size() && m_tty_size != tty->fd.size())
	{
		std::string buf;
		{
			std::lock_guard lock(tty->mtx);
			...
			tty->fd.read(buf, tty->fd.size() - m_tty_size);
			tty->fd.seek(pos);
			m_tty_size = tty->fd.size();
		}
		...
		if (m_TTYAct->isChecked())
		{
				...
				start_line = std::exchange(end_line, std::min<usz>(buf.size(), buf.find_first_of('\n', end_line + 1))) + 1)
			{
				...
				if (steady_clock::now() >= start + 4ms) break;
			}
		}
	}
